...
import com.uy.atiliator.sie.inspeccion.config.KafkaProperties;
...
class InspeccionKafkaResourceIT {
    ...
    void setup() {
        ...
        InspeccionKafkaResource kafkaResource = new InspeccionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/inspeccion-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/inspeccion-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
